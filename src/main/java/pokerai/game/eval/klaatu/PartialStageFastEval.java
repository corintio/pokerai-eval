...
public final class PartialStageFastEval {
	...
	public final void setCard3(int card) {
		...
		rval123 = fst7123[((card1 & RANK_MASK) << (ONE_CARD_WIDE_RANK_SHIFT + WIDE_RANK_SHIFT)) + 
			   ((card2 >> LOG2_SUIT_COUNT) << WIDE_RANK_SHIFT) + 
	...
	public final int setHand7(int card) {
		...
		int suit = (((sval7 & (CLUB_FLUSH | DIAMOND_FLUSH)) != 0) ? 
		    (((sval7 & CLUB_FLUSH) != 0) ? CLUBS : DIAMONDS) :
		    (((sval7 & SPADE_FLUSH) != 0) ? SPADES : HEARTS));
		int suitedCards = 0;
		if ((card1 & SUIT_MASK) == suit) suitedCards |= (1 << (card1 >> LOG2_SUIT_COUNT));
		if ((card2 & SUIT_MASK) == suit) suitedCards |= (1 << (card2 >> LOG2_SUIT_COUNT));
		if ((card3 & SUIT_MASK) == suit) suitedCards |= (1 << (card3 >> LOG2_SUIT_COUNT)); 
		if ((card4 & SUIT_MASK) == suit) suitedCards |= (1 << (card4 >> LOG2_SUIT_COUNT));
		if ((card5 & SUIT_MASK) == suit) suitedCards |= (1 << (card5 >> LOG2_SUIT_COUNT));
		if ((card6 & SUIT_MASK) == suit) suitedCards |= (1 << (card6 >> LOG2_SUIT_COUNT));
		if ((card & SUIT_MASK) == suit)  suitedCards |= (1 << (card >> LOG2_SUIT_COUNT));
	...
	if ((sval & FLUSH_MASK) == 0) {
	    ...
	    int rval = fst7123[((card1 & RANK_MASK) << (ONE_CARD_WIDE_RANK_SHIFT + WIDE_RANK_SHIFT)) + 
			   ((card2 >> LOG2_SUIT_COUNT) << WIDE_RANK_SHIFT) + 
			   (card3 >> LOG2_SUIT_COUNT)];
	    rval = fst77[(fst76[(fst75[fst74[rval + (card4 >> LOG2_SUIT_COUNT)]
+ 
	...
	// Find the cards that form the flush
	int suit = (((sval & (CLUB_FLUSH | DIAMOND_FLUSH)) != 0) ? 
	...
	if ((card5 & SUIT_MASK) == suit)  suitedCards |= (1 << (card5 >> LOG2_SUIT_COUNT));
	if ((card6 & SUIT_MASK) == suit)  suitedCards |= (1 << (card6 >> LOG2_SUIT_COUNT));
	if ((card7 & SUIT_MASK) == suit)  suitedCards |= (1 << (card7 >> LOG2_SUIT_COUNT));
	return flush7[suitedCards];
    }
	...
	int rval = fst55[fst54[fst5123[((card1 & RANK_MASK) << (ONE_CARD_WIDE_RANK_SHIFT + WIDE_RANK_SHIFT)) + 
				       ((card2 >> LOG2_SUIT_COUNT) << WIDE_RANK_SHIFT) + 
				       (card3 >> LOG2_SUIT_COUNT)] +
			       (card4 >> LOG2_SUIT_COUNT)] + 
	...
	int suitedCards = 
	    (1 << (card1 >> LOG2_SUIT_COUNT)) |
	    (1 << (card2 >> LOG2_SUIT_COUNT)) |
	    (1 << (card3 >> LOG2_SUIT_COUNT)) |
	    (1 << (card4 >> LOG2_SUIT_COUNT)) |
	    (1 << (card5 >> LOG2_SUIT_COUNT));
    ...
    static String[] _cardStrings = {
	"bad1", "bad2", "Deuce", "Three", "Four", "Five", "Six", "Seven", "Eight", "Nine", "Ten",
	"Jack", "Queen", "King", "Ace"};
